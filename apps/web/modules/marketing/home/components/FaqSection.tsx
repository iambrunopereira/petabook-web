...
export function FaqSection({ className }: { className?: string }) {
  const items = [
    {
      question: "O que são Hotéis Parceiros Tripet.pt?",
      answer:
        "A Tripet.pt colabora com hotéis caninos de confiança, onde os visitantes do nosso website podem realizar pré-reservas diretamente na nossa plataforma. Estes hotéis são cuidadosamente selecionados pela Tripet.pt, pois a nossa equipa mantém um contacto próximo e regular com os proprietários e colaboradores desses estabelecimentos. Confie na qualidade e segurança dos nossos parceiros e faça a sua reserva com a Tripet.pt para garantir o melhor cuidado para o seu cão.",
    },
    {
      question: "É perigoso deixar o meu cão num hotel?",
      answer:
        "Se deixar num estabelecimento de confiança, como os que são apresentados na Tripet.pt, não há qualquer tipo de risco em deixar o seu cão num hotel canino. Várias medidas são tomadas para minimizar a má adaptação do seu animal com os restantes. Os profissionais neste setor estão treinados e habituados em facilitar a socialização entre os cães. Por norma, é habitual darem feedback aos donos sobre o processo de integração do cão no hotel.",
    },
    {
    ...
    },
    {
      ...
      answer:
        "É muito frequente os Hotéis Caninos disponibilizarem-se para ir partilhando fotografias do seu cão. Pode perguntar ou pedir ajuda de um especialista do Tripet.pt para poder ter essa questão assegurada. Além disso, as recomendações e comentários de um estabelecimento devem ser critérios quando está a escolher a estadia do seu animal de estimação.",
    },
    {
      ...
      <div className="container max-w-5xl">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {items.map((item, i) => (
            <div
              key={`faq-item-${i}`}
              className="rounded-lg border p-4 lg:p-6 transition-all duration-300"
            >
              <button
